...
- The current bounded interaction-assimilation slice is an upstream Codex-style conversation-summary, turn-selection, thread-read, and turn-lifecycle preview inside `vector-main-interaction-program`, not a claim that the full upstream base is merged.
