...
- The current bounded interaction-assimilation slice is an upstream Codex-style conversation-summary, selected-turn detail, tool/payload preview, execution-result and approval-resolution continuity, approval/artifact continuity, thread-read, and turn-lifecycle preview inside `vector-main-interaction-program`, not a claim that the full upstream base is merged.
